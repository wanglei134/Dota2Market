...
<html>
  ...
  &nbsp;
	...
	<input type="button" value="点击查询库存" id="fetch" onclick="fetchI()"/>
	<br>
	<span id="info"></span>
	&nbsp;&nbsp;
	</form>	
...
<div>
	<div>
		...
		<div>
			...
			<div id="myDiv" style="float:left;width:80%">
				<div id="totalInfo">
				<span>Total:</span><span style="color:red">$${total_price }</span><span>(</span>${count }<span> items)</span>
				<span>税后价格:<span style="color:red">$${afterTaxPrice }</span></span>
				</div>
				<br>	
				<c:if test="${noPriceCount>0}">
					<span>库存有<span style="color:red">${noPriceCount }件</span>物品未能查询到价格,请稍后...</span><br>
				</c:if>									
			...
			<table align="center"  cellpadding="10" cellspacing="0">
				...
				<tr height="10px">
					...
					<c:if test="${pageNum gt 1 }">
						<a  href="goods.do?operate=toDotaManager&type=${type}&steamId=${steamId}">首页</a>
						<a  href="goods.do?operate=toDotaManager&pageNum=${pageNum-1 }&type=${type}&steamId=${steamId}">上一页</a>
					</c:if>
					<c:if test="${pageNum eq maxPage }">
						<a>下一页</a> <a>尾页</a> 
					</c:if>
					<c:if test="${pageNum lt maxPage }">
						<a href="goods.do?operate=toDotaManager&pageNum=${pageNum+1 }&type=${type}&steamId=${steamId}">下一页</a>
						<a href="goods.do?operate=toDotaManager&pageNum=${maxPage }&type=${type}&steamId=${steamId}">尾页</a>
					</c:if>
					</td>
					<td >
						 第<input type="text" style="width: 20px" maxlength="2"  name="pageNum" value="${pageNum }" id="pageNum" >/${maxPage }页 
						 <input type="submit" value="GO" onClick="return checkNum()">
					</td>
				</tr>
			</table>
			</div>
	...
	<logic:notEmpty name="user" scope="session">
			欢迎您,<span style="color:red;font-size:20px">${user.personaname }</span>	<br>
			<a href="goods.do?operate=logout" >退出登录</a>				
	</logic:notEmpty>
	<logic:empty name="user" scope="session">
			<span style="color:red;">登陆后自动填写steamID</span>		
	<br>
	<a href="goods.do?operate=login" style="text-decoration:none;">
	 ...
	 <table border="1">
		...
		<tr>
		  ...
		  <td id="np4">$${price_object[3] }</td>
		   ...
		   <c:if test="${type eq 570}">
		 	 <td ><input type="text" id="b4" value="4.4" style="width:40px"/></td>
		  </c:if>
		  <c:if test="${type eq 730}">
		 	 <td ><input type="text" id="b4" value="4.2" style="width:40px"/></td>
		  </c:if>
		</tr>
	</table>
	<br>
	<input type="button" style="width:65" value="隐藏估价" id="hiddenvalue" onclick="hiddenvalueI()"/>	
	<input type="button" style="width:65" value="显示税后" id="aftertaxprice" onclick="aftertaxI()"/>
	<input type="button" style="width:65" value="计算估价" id="calc" onclick="calcI()"/>	
	</logic:notEmpty>	
	<br>
	<br>
